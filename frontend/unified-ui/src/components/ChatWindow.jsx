...
export default function ChatWindow() {
  ...
  if(label==="Check Balance") {
    const bal = await fetch("http://localhost:8000/transfer/balance?user_id=customer" , {
                      method:"GET",
                      })
  ...
  if (label === "Transfer Funds") {
    ...
    const res = await fetch("http://localhost:8000/beneficiary/list", {
    headers: {
  ...
  return;
   ...
   const handleTransferFlow = async (input, selectedBeneficiary) => {
    ...
    if (amount <= 0) {
        ...
        const res = await fetch("http://localhost:8000/transfer/validate", {
            method: "POST",
        ...
        const exec = await fetch("http://localhost:8000/transfer/execute", {
            method: "POST",
            headers: {"Content-Type" : "application/json"},
   ...
   const handleAddBeneficiaryFlow = async (input) => {
        ...
        await fetch("http://localhost:8000/beneficiary/add", {
            method: "POST",
            headers: {
  ...
  if (intent === "CHECK_BALANCE") {
    const bal = await fetch(
      "http://localhost:8000/transfer/balance?user_id=customer"
    );
    const data = await bal.json();
    addBotMessage(`Available balance is ${data.balance}`);
    return;
  ...
  try {
    const res = await fetch(
      "http://localhost:8000/transfer/history?user_id=customer&limit=10",
      { method : "GET",
        headers: {
          Authorization: "Basic " + btoa("customer:pass")
        }
